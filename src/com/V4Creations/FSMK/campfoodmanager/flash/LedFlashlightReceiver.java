...
import static com.V4Creations.FSMK.campfoodmanager.flash.Constants.SDK_VERSION;
...
public class LedFlashlightReceiver extends BroadcastReceiver {
	...
	// UI sends this action in oder to control flashlight state (uses
	// EXT_ENABLED)
	...
	public static int detectLedFlashlightType(Context context) {
		...
		int type = flashlight == null ? -1 /* not supported */: flashlight
				.getType();
		SharedPreferences prefs = context.getSharedPreferences(
				Constants.PREFS_COMMON, Context.MODE_WORLD_WRITEABLE);
	...
	private static Flashlight getFlashlight(Context context) {
		...
		if (FLASHLIGHT == null) {
			synchronized (LedFlashlightReceiver.class) {
				if (FLASHLIGHT == null) {
					SharedPreferences prefs = context.getSharedPreferences(
							Constants.PREFS_COMMON,
							Context.MODE_WORLD_WRITEABLE);
					int type = prefs.getInt(Constants.PREF_FLASHLIGHT_TYPE, -2); /*
																				 * -
																				 * 2
																				 * means
																				 * not
																				 ...
																				 */
					if (type == -2) {
						// this can happen on first start after upgrate to 1.9.4
						// p1
						type = detectLedFlashlightType(context);
					}
					switch (type) {
					case Flashlight.TYPE_MOTO21:
						FLASHLIGHT = new Moto21LedFlashlight();
						break;
					case Flashlight.TYPE_HTC:
						FLASHLIGHT = new HtcLedFlashlight();
						break;
					case Flashlight.TYPE_FROYO:
						FLASHLIGHT = new FroyoLedFlashlight();
						break;
					case Flashlight.TYPE_DROID22:
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (ACTION_CONTROL_FLASHLIGHT.equals(action)) {
			...
			if (enabled) {
				...
				} // otherwise do nothing (no need to update UI as it stays
					// switched off anyway)
			...
			} // else not further processing needed as flashlight is already
				// switched off
		}
	}
	...
	private static void enableFlashlight(Context context, boolean enabled) {
		...
		try {
			...
			// this can happen if flashlight stops working after a system
			// software update
			SharedPreferences props = context.getSharedPreferences(
					Constants.PREFS_COMMON, Context.MODE_WORLD_WRITEABLE);
			props.edit().putInt(Constants.PREF_FLASHLIGHT_TYPE, 0).commit(); // screen
																				// flashlight
		}
	}
	...
	private static int getWarnCount(Context context) {
		...
		@SuppressWarnings("deprecation")
		SharedPreferences props = context.getSharedPreferences(
				Constants.PREFS_RUNTIME, Context.MODE_WORLD_WRITEABLE);
